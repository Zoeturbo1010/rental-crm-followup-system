...
export default function Home() {
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <nav>{nav.map(([label, icon]) => <button key={label} className={page === label || (label === "客户管理" && page === "客户详情") ? "active" : ""} onClick={() => openPage(label)}><i>{icon}</i>{label}{label === "工作台" && <em>{riskCount}</em>}</button>)}</nav>
