...
type FollowUp = { time: string; title: string; desc: string; result?: string; reason?: string; nextAt?: string; stage?: string; type?: string; intent?: string; kind?: "system" | "followup" };
const isFollowUpEntry = (item: FollowUp) => item.kind === "followup" || (item.kind !== "system" && !item.title.includes("建立客户档案"));
...
const followupStages = ["看房前", "看房后", "成交后"];
const followupTypes = ["电话", "企微", "其他"];
const intentLevels = ["高意向", "中意向", "低意向"];
const replaceStageTag = (tags: string[], stage: string) => Array.from(new Set([...tags.filter(tag => !columns.includes(tag)), stage]));
const boardColumns = [
...
const tagFilters = ["全部标签", "未看房", "已看房", "待跟进", "待推荐", "待看房", "已成交", "高紧急", "高意向", "中意向", "低意向", "今日看房", "客户未回复", "暂无需求", "半月后入住", "没有合适房源", "预算不匹配", "已加企微", "仅浏览用户", "已收藏用户", "房子不合适", "二次推荐", "待回访", "待预约", "平台内私信", "电话沟通", "企微沟通", "别处成交"];
const nav = ["工作台", "客户管理", "跟进工作台", "看房管理", "成交管理", "数据分析"];

function NavIcon({name}:{name:string}) {
  const common = {fill:"none", stroke:"currentColor", strokeWidth:1.8, strokeLinecap:"round" as const, strokeLinejoin:"round" as const};
  if (name === "工作台") return <svg className="nav-glyph" viewBox="0 0 24 24" aria-hidden="true"><path {...common} d="M4 13.5 12 6l8 7.5"/><path {...common} d="M6.5 11.5V20h11v-8.5M10 20v-5h4v5"/></svg>;
  if (name === "客户管理") return <svg className="nav-glyph" viewBox="0 0 24 24" aria-hidden="true"><circle {...common} cx="12" cy="8" r="3.2"/><path {...common} d="M5.5 19c.7-3.5 3-5.3 6.5-5.3s5.8 1.8 6.5 5.3"/></svg>;
  if (name === "跟进工作台") return <svg className="nav-glyph" viewBox="0 0 24 24" aria-hidden="true"><circle {...common} cx="12" cy="12" r="7.5"/><path {...common} d="M12 8v4l2.8 1.8"/><path {...common} d="M18.2 5.8 20 4"/></svg>;
  if (name === "看房管理") return <svg className="nav-glyph" viewBox="0 0 24 24" aria-hidden="true"><path {...common} d="M4.5 11.5 12 5l7.5 6.5V20h-15z"/><path {...common} d="M9 20v-5h6v5M8 10h.01M16 10h.01"/></svg>;
  if (name === "成交管理") return <svg className="nav-glyph" viewBox="0 0 24 24" aria-hidden="true"><path {...common} d="M5 7h14v12H5zM8 4h8v3"/><path {...common} d="m8.5 13 2.2 2.2 4.8-5"/></svg>;
  return <svg className="nav-glyph" viewBox="0 0 24 24" aria-hidden="true"><path {...common} d="M5 19V9M12 19V5M19 19v-7"/><path {...common} d="M3.5 19.5h17"/></svg>;
}
...
const defaultNextDateTime = (hours = 24) => new Date(Date.now() + hours * 60 * 60 * 1000).toISOString().slice(0, 16);
...
export default function Home() {
  ...
  const [followupStageFilter, setFollowupStageFilter] = useState("全部阶段");
  const [followupIntentFilter, setFollowupIntentFilter] = useState("全部意向");
  ...
  const followupClients = useMemo(() => clientRecords.filter(client => {
    const latest = (followUps[client.id] ?? []).find(isFollowUpEntry);
    return (followupStageFilter === "全部阶段" || latest?.stage === followupStageFilter) && (followupIntentFilter === "全部意向" || latest?.intent === followupIntentFilter);
  }), [clientRecords, followUps, followupStageFilter, followupIntentFilter]);
  const riskCount = clientRecords.filter(client => {
    if (client.stage === "已成交") return false;
    const due = client.next.includes("已超时") || client.next.includes("今天") || Boolean(client.nextAt && new Date(client.nextAt).getTime() <= Date.now());
    return client.tags.includes("跟进超时") || (reminderRules.firstResponse && client.tags.includes("首响超时")) || (reminderRules.recommendation && due && client.stage === "待推荐") || (reminderRules.viewingFollowup && due && client.tags.includes("需回访")) || (reminderRules.viewingConfirmation && due && client.tags.includes("待预约"));
  }).length;
  ...
  const updateClient = (nextClient:Client) => { setClientRecords(current => current.map(item => item.id === nextClient.id ? nextClient : item)); setSelected(current => current?.id === nextClient.id ? nextClient : current); if(nextClient.stage === "已成交" || (nextClient.nextAt && new Date(nextClient.nextAt).getTime() > Date.now())) setNotifications(current => current.filter(item => !item.startsWith(`${nextClient.name}：`))); };
  const syncFollowUp = (client:Client, desc:string, tags:string[], stage:string, nextAt:string, result:string, reason?:string, followupStage?:string, followupType?:string, intent?:string) => {
    const extras = ["已加企微"];
    ...
    if (intent) extras.push(intent);
    if (followupType === "电话") extras.push("电话沟通");
    if (followupType === "企微") extras.push("企微沟通");
    const nextStageByResult = result.includes("不合适") || result.includes("待推荐") ? "待推荐" : (result.includes("待预约") || result.includes("安排带看") ? "待看房" : result.includes("成交") ? "已成交" : stage);
    const nextStage = followupStage ? (client.stage === "已成交" ? "已成交" : "待推荐") : nextStageByResult;
    const finalDesc = reason ? `${desc}（原因：${reason}）` : desc;
    const cleanedTags = tags.filter(tag => !["跟进超时", "首响超时", ...intentLevels].includes(tag));
    const updated = {...client, stage:nextStage, viewed: nextStage === "已成交" ? true : client.viewed, next:taskTimeLabel(nextAt), nextAt, last:finalDesc, tags:replaceStageTag([...cleanedTags,...extras], nextStage)};
    updateClient(updated);
    setFollowUps(current => ({...current, [client.id]: [{time:"刚刚", title:"林晓 · 跟进记录", desc:finalDesc, result, reason, nextAt, stage:followupStage ?? (client.viewed ? "看房后" : client.stage === "已成交" ? "成交后" : "看房前"), type:followupType ?? "企微", intent: intent ?? (result.includes("高意向") ? "高意向" : "中意向"), kind:"followup"}, ...(current[client.id] ?? [])]}));
    setNotifications(current => [`${client.name}：${result}，下次跟进 ${taskTimeLabel(nextAt)}`, ...current.filter(item => !item.startsWith(`${client.name}：`))]);
    return updated;
  };
  ...
  const createDeal = (client:Client, property:string) => { const record:DealRecord={id:`CJ${Date.now().toString().slice(-8)}`,clientId:client.id,clientName:client.name,property,rent:"¥8,800/月",commission:"¥8,800",status:"待结算",owner:client.owner,date:"刚刚"}; setDealRecords(current=>[record,...current]); updateClient({...client,stage:"已成交",viewed:true,next:"已完成",last:`已成交：${property}`,tags:replaceStageTag([...client.tags,"已签约"],"已成交")}); setNotifications(current=>[`${client.name}：成交记录已创建，待佣金结算`,...current]); alert("成交已同步至成交管理与客户时间线"); openPage("成交管理"); };
  ...
  return <main className="app-shell">
    <aside className="sidebar" inert={overlayOpen}>
      ...
      <nav aria-label="业务导航">
        ...
        {nav.map(label => {
          const active = page === label || (label === "客户管理" && page === "客户详情") || (label === "看房管理" && page === "预约看房") || (label === "工作台" && page === "超时任务");
          return <button key={label} className={active ? "active" : ""} aria-current={active ? "page" : undefined} aria-label={label} title={label} onClick={() => openPage(label)}><NavIcon name={label}/><span className="nav-label">{label}</span>{label === "工作台" && riskCount > 0 && <em>{riskCount}</em>}</button>;
        })}
      </nav>
    ...
    <section className="main-area" inert={overlayOpen}>
      ...
      <div className="content">{page === "工作台" && <Dashboard onPick={setSelected} onNavigate={openPage} alert={alert} riskCount={riskCount} clients={clientRecords} appointments={appointments} deals={dealRecords} />}
        ...
        {page === "跟进工作台" && <><FollowUpFilters stage={followupStageFilter} intent={followupIntentFilter} onStageChange={setFollowupStageFilter} onIntentChange={setFollowupIntentFilter} /><FollowUpWorkspaceV2 clients={followupClients} followUps={followUps} onPick={setSelected} onSave={(client,desc,nextAt,result,followupStage,followupType,intent)=>syncFollowUp(client,desc,client.tags,client.stage,nextAt,result,undefined,followupStage,followupType,intent)} /></>}
        {page === "看房管理" && <ViewingV2 records={appointments} clients={clientRecords} onPick={setSelected} onCreate={() => startAppointment(clientRecords.find(client=>client.stage !== "已成交") ?? clientRecords[0])} onComplete={(record) => { const client=clientRecords.find(item=>item.id===record.clientId); if(client) { const followupAt = new Date(Date.now() + 2 * 60 * 60 * 1000).toISOString().slice(0,16); syncFollowUp({...client,viewed:true}, `已完成看房：${record.property}，待回访`, Array.from(new Set([...client.tags,"看房后"])), "待推荐", followupAt, "看房完成待回访"); setAppointments(current=>current.map(item=>item.id===record.id?{...item,status:"已完成"}:item)); setNotifications(current=>[`${client.name}：看房已完成，请在 2 小时内回访`,...current]); alert("看房结果已同步到客户跟进，并创建回访提醒"); } }} />}
        ...
        {page === "数据分析" && <AnalyticsDashboard clients={clientRecords} deals={dealRecords} viewings={appointments} followUps={followUps} />}
        {page === "超时任务" && <OverdueTasks clients={clientRecords} onPick={setSelected} onBack={() => setPage("工作台")} alert={alert} riskCount={riskCount} onSettings={() => setReminderSettingsOpen(true)} />}
      </div>
    </section>
    ...
    {clientModal && <NewClientModalV2 onClose={() => setClientModal(false)} onSave={(client) => { const enriched = {...client, stage:"待跟进", tags:replaceStageTag(Array.from(new Set([...client.tags,"已加企微"])),"待跟进")}; setClientRecords(current => [enriched, ...current]); setFollowUps(current => ({...current, [enriched.id]: [{time:"刚刚",title:"林晓 · 建立客户档案",desc:enriched.last,result:"已加企微，待首次跟进",kind:"system"}]})); setNotifications(current => [`${enriched.name}：已创建下次跟进任务（${enriched.next}）`, ...current]); setClientModal(false); openPage("跟进工作台"); alert("客户已加入跟进工作台，并创建应用内提醒"); }} />}
...
function Dashboard({ onPick, onNavigate, alert, riskCount, clients, appointments, deals }: { onPick: (c: Client) => void; onNavigate: (p:string) => void; alert:(message:string)=>void; riskCount:number; clients:Client[]; appointments:ViewingRecord[]; deals:DealRecord[] }) {
  ...
  const urgentClients = activeClients.filter(client => client.next.includes("超时") || client.tags.includes("跟进超时") || client.tags.includes("首响超时") || client.tags.includes("需回访"));
...
function OverdueTasks({clients,onPick,onBack,alert,riskCount,onSettings}:{clients:Client[];onPick:(c:Client)=>void;onBack:()=>void;alert:(message:string)=>void;riskCount:number;onSettings:()=>void}) {
  const rows = clients.filter(client => client.stage !== "已成交" && (client.next.includes("超时") || client.tags.includes("跟进超时") || client.tags.includes("首响超时") || client.tags.includes("需回访") || client.tags.includes("待预约"))).map(client => {
    const type = client.tags.includes("需回访") ? "看房后未回访" : client.tags.includes("首响超时") ? "首响超时" : client.tags.includes("待预约") ? "看房确认" : "推荐超时";
    const note = type === "看房后未回访" ? "客户反馈尚未记录，系统连续提醒" : type === "首响超时" ? "IM 进线后尚未完成需求确认" : type === "看房确认" ? "预约看房尚未完成二次确认" : "客户提出新条件，尚未补充推荐方案";
    return [client, client.next.includes("超时") ? client.next.replace("已超时", "") : "待处理", type, note] as const;
  });
...
function FollowUpFilters({stage,intent,onStageChange,onIntentChange}:{stage:string;intent:string;onStageChange:(value:string)=>void;onIntentChange:(value:string)=>void}) { return <div className="followup-filter-bar"><div><span>跟进筛选</span><small>按最近一次跟进记录定位客户</small></div><label>跟进阶段<select value={stage} onChange={event=>onStageChange(event.target.value)}><option>全部阶段</option><option>看房前</option><option>看房后</option><option>成交后</option></select></label><label>意向等级<select value={intent} onChange={event=>onIntentChange(event.target.value)}><option>全部意向</option><option>高意向</option><option>中意向</option><option>低意向</option></select></label><button onClick={()=>{onStageChange("全部阶段");onIntentChange("全部意向")}}>重置筛选</button></div>; }

function FollowUpWorkspaceV2({clients,followUps,onPick,onSave}:{clients:Client[];followUps:Record<number,FollowUp[]>;onPick:(client:Client)=>void;onSave:(client:Client,desc:string,nextAt:string,result:string,followupStage:string,followupType:string,intent:string)=>void}) {
  const tasks = [...clients].sort((a,b)=>a.next.includes("已超时") ? -1 : b.next.includes("已超时") ? 1 : 0);
  const [activeId,setActiveId]=useState(tasks[0]?.id ?? clients[0]?.id ?? 0);
  const [draft,setDraft]=useState("");
  const [result,setResult]=useState("已联系等待反馈");
  const [followupStage,setFollowupStage]=useState("看房前");
  const [followupType,setFollowupType]=useState("企微");
  const [intent,setIntent]=useState("中意向");
  const [nextTime,setNextTime]=useState(defaultNextDateTime(24));
  const setQuickNextTime = (days:number) => setNextTime(new Date(Date.now() + days * 86400000).toISOString().slice(0,16));
  const active=clients.find(c=>c.id===activeId) ?? tasks[0] ?? clients[0];
  const records = active ? (followUps[active.id] ?? []) : [];
  useEffect(()=>{if(!active)return;setFollowupStage(active.stage === "已成交" ? "成交后" : active.viewed ? "看房后" : "看房前");setIntent(intentLevels.find(item=>active.tags.includes(item)) ?? "中意向");},[activeId]);
  if (!active) return <section className="panel followup-empty-state"><h2>暂无匹配客户</h2><p>请调整跟进阶段或意向等级筛选条件。</p></section>;
  return <><div className="page-heading"><div><h1>跟进工作台</h1><p>所有新增客户、下次跟进时间与看房后的回访任务都会自动汇总到这里。</p></div><button className="create" onClick={()=>tasks[0] && onPick(tasks[0])}>处理下一条待办</button></div>
    ...
    <section className="followup-layout"><aside className="followup-queue"><header><div><span>FOLLOW-UP QUEUE</span><h2>统一待办队列</h2></div><button>按下次时间</button></header><div className="queue-tabs"><b>全部 <i>{tasks.length}</i></b><span>今天 <i>{tasks.filter(client=>client.next.includes("今天")).length}</i></span></div>{tasks.map((client,index)=><button className={activeId===client.id?"queue-item selected":"queue-item"} onClick={()=>setActiveId(client.id)} key={client.id}><div className={index===0?"queue-time q0":"queue-time"}>{client.next.includes("已超时")?"超时":client.next.replace("今天 ","")}</div><Avatar value={client.initials}/><div><b>{client.name}<small>{client.viewed?"已看房":"未看房"} · {client.stage}</small></b><p>{client.last}</p></div><span>›</span></button>)}</aside><section className="followup-editor"><header><div className="editor-client"><Avatar value={active.initials} large/><div><h2>{active.name} <Badge tone={active.viewed?"purple":"blue"}>{active.viewed?"已看房":"未看房"} · {active.stage}</Badge></h2><p>{active.phone}　·　{active.area}　·　{active.budget}</p></div></div><button onClick={()=>onPick(active)}>详情 →</button></header><div className="editor-body"><div className="editor-context"><span>当前任务</span><b>{active.last}</b><div>{active.tags.map(tag=><i key={tag}>#{tag}</i>)}</div></div><div className="record-form"><h3>填写本次跟进</h3><div className="followup-meta-row"><label>跟进阶段<select value={followupStage} onChange={e=>setFollowupStage(e.target.value)}><option>看房前</option><option>看房后</option><option>成交后</option></select></label><label>跟进类型<select value={followupType} onChange={e=>setFollowupType(e.target.value)}><option>电话</option><option>企微</option><option>其他</option></select></label><label>意向等级<select value={intent} onChange={e=>setIntent(e.target.value)}><option>高意向</option><option>中意向</option><option>低意向</option></select></label></div><div className="result-row"><label>跟进结果<select value={result} onChange={e=>setResult(e.target.value)}><option>已联系等待反馈</option><option>已确认需求待推荐</option><option>客户有意向待预约看房</option><option>仅浏览用户安排带看</option><option>已收藏用户安排带看</option><option>房子不合适，二次推荐</option><option>客户暂缓</option><option>无法联系</option></select></label><label>下次跟进时间<input type="datetime-local" value={nextTime} onChange={e=>setNextTime(e.target.value)} /></label></div><div className="inline-quick-time"><span>快速设置：</span>{[5,10,15,30].map(days=><button type="button" key={days} onClick={()=>setQuickNextTime(days)}>+{days}天</button>)}</div><label className="record-note">跟进内容<textarea value={draft} onChange={e=>setDraft(e.target.value)} placeholder="记录客户的最新反馈、需求变化、房源偏好或下一步计划..." /></label><div className="form-save"><span>保存后将同步客户状态、标签与提醒</span><button className="create" onClick={()=>{if(draft.trim()){onSave(active,draft.trim(),nextTime,result,followupStage,followupType,intent);setDraft("");}}}>保存并同步</button></div></div><div className="recent-records"><h3>最近跟进记录 <em>{records.filter(isFollowUpEntry).length} 条</em></h3>{records.filter(isFollowUpEntry).length ? records.filter(isFollowUpEntry).map((item,index)=><div className="followup-history-item" key={`${item.time}-${index}`}><i></i><div><p><b>{item.result || "客户跟进"}</b>{item.stage && <span> · {item.stage}</span>}{item.intent && <span> · {item.intent}</span>}{item.type && <span> · {item.type}</span>}</p><strong>{item.desc}</strong><small>{item.time}　·　林晓　·　下一步：{item.nextAt ? taskTimeLabel(item.nextAt) : active.next}</small></div></div>) : <div className="followup-history-empty"><i></i><p>暂无历史跟进记录</p><small>保存本次跟进后，会在这里形成完整时间线</small></div>}</div></div></section></section></>;
}
...
function NewClientModalV2({onClose,onSave}:{onClose:()=>void;onSave:(client:Client)=>void}) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><form className="new-client-modal" role="dialog" aria-modal="true" aria-label="新增客户" onMouseDown={event=>event.stopPropagation()} onSubmit={submit}><header><div><h2>新增客户</h2><p>首次录入时完成客户需求、来源和初始标签标注。</p></div><button className="close" aria-label="关闭弹窗" type="button" onClick={onClose}>×</button></header><div className="form-scroll">
    ...
    <FormSection title="初始业务标签"><div className="client-tag-picker"><p>新客户自动进入“待跟进”；第一次保存跟进记录后，系统会自动切换为“待推荐”。其他标签可在跟进记录中补充。</p><div>{["待跟进", ...tagFilters.slice(7)].map(tag=><button type="button" className={initialTags.includes(tag)?"selected":""} onClick={()=>toggle(tag)} key={tag}>#{tag}</button>)}</div></div></FormSection>
...
function ClientDrawerV2({client,followUps,appointments,onAddFollowUp,onUpdateClient,onRecommend,onClose,onDeal,onAppointment,alert}:{client:Client;followUps:FollowUp[];appointments:ViewingRecord[];onAddFollowUp:(entry:FollowUp)=>void;onUpdateClient:(client:Client)=>void;onRecommend:()=>void;onClose:()=>void;onDeal:()=>void;onAppointment:()=>void;alert:(x:string)=>void}) {
  ...
  return <div className="drawer-backdrop" onMouseDown={onClose}><aside className="drawer" role="dialog" aria-modal="true" aria-label="客户详情" onMouseDown={event=>event.stopPropagation()}><header><div className="drawer-person"><Avatar value={client.initials} large/><div><h2>{client.name} <Badge>{client.stage}</Badge></h2><p>{client.phone}　·　{client.owner} 负责</p></div></div><button className="close" aria-label="关闭弹窗" onClick={onClose}>×</button></header>
    <div className="drawer-info"><span>下次跟进 <b className="blue-text">{client.next}</b></span><span>意向区域 <b>{client.area}</b></span><span>预算 <b>{client.budget}</b></span><label>看房状态<select value={client.viewed?"已看房":"未看房"} disabled={client.stage==="已成交"} onChange={event=>{onUpdateClient({...client,viewed:event.target.value==="已看房"});alert("看房状态已更新");}}><option>未看房</option><option>已看房</option></select></label></div>
    ...
    {adding && <FollowUpModalV2 client={client} onClose={()=>setAdding(false)} onSave={(desc,tags,stage,nextAt,result,reason,followupStage,followupType,intent)=>{const nextStage=client.stage==="已成交"?"已成交":"待推荐"; onAddFollowUp({time:"刚刚",title:"林晓 · 跟进记录",desc, result, reason, nextAt, stage:followupStage, type:followupType, intent, kind:"followup"}); onUpdateClient({...client,tags:replaceStageTag([...tags.filter(tag=>!['跟进超时','首响超时',...intentLevels].includes(tag)),"已加企微", ...(result.includes("不合适")?["房子不合适","二次推荐"]:[]), ...(result.includes("待预约")?["待预约"]:[]), intent, ...(followupType==="电话"?["电话沟通"]:followupType==="企微"?["企微沟通"]:[])],nextStage),stage:nextStage,next:taskTimeLabel(nextAt),nextAt,last:reason?`${desc}（原因：${reason}）`:desc}); setAdding(false); alert("跟进结果已同步至客户管理、跟进工作台与提醒队列");}} onAppointment={onAppointment} />}
  </aside></div>;
}

function FollowUpModalV2({client,onClose,onSave,onAppointment}:{client:Client;onClose:()=>void;onSave:(desc:string,tags:string[],stage:string,nextAt:string,result:string,reason:string|undefined,followupStage:string,followupType:string,intent:string)=>void;onAppointment:()=>void}) {
  const [content,setContent] = useState("");
  const [result,setResult] = useState("已联系，等待客户反馈");
  const [selectedTags,setSelectedTags] = useState(client.tags);
  const [followupStage,setFollowupStage] = useState(client.stage === "已成交" ? "成交后" : client.viewed ? "看房后" : "看房前");
  const [followupType,setFollowupType] = useState("企微");
  const [intent,setIntent] = useState(intentLevels.find(item=>client.tags.includes(item)) ?? "中意向");
  const [next,setNext] = useState(defaultNextDateTime(24));
  const [reason,setReason] = useState("房子本身装修/格局不合适");
  const availableTags = tagFilters.slice(7).filter(tag=>!intentLevels.includes(tag));
  const toggleTag = (tag:string) => setSelectedTags(current => current.includes(tag) ? current.filter(item=>item!==tag) : [...current,tag]);
  const setQuickNext = (days:number) => setNext(new Date(Date.now() + days * 86400000).toISOString().slice(0,16));
  return <div className="followup-backdrop" onMouseDown={onClose}><form className="followup-modal" role="dialog" aria-modal="true" aria-label="新增跟进记录" onMouseDown={event=>event.stopPropagation()} onSubmit={event=>{event.preventDefault(); onSave(content.trim() || "已完成客户跟进，并设置了下一步处理计划。",selectedTags,client.stage,next,result,result.includes("不合适") ? reason : undefined,followupStage,followupType,intent); if(result.includes("待预约") || result.includes("安排带看")) onAppointment();}}><header><div><h3>新增跟进记录</h3><p>{client.name} · 保存后客户标签自动切换为“待推荐”</p></div><button type="button" className="close" aria-label="关闭弹窗" onClick={onClose}>×</button></header>
    <div className="followup-meta-grid"><label><span>跟进阶段 <i>*</i></span><select value={followupStage} onChange={event=>setFollowupStage(event.target.value)}>{followupStages.map(item=><option key={item}>{item}</option>)}</select></label><label><span>跟进类型 <i>*</i></span><select value={followupType} onChange={event=>setFollowupType(event.target.value)}>{followupTypes.map(item=><option key={item}>{item}</option>)}</select></label><label><span>意向等级 <i>*</i></span><select value={intent} onChange={event=>setIntent(event.target.value)}>{intentLevels.map(item=><option key={item}>{item}</option>)}</select></label></div>
    ...
    <div className="followup-tag-field"><span>业务标签</span><small>可补充需求与来源标签；流程标签由系统自动更新。</small><div>{availableTags.map(tag=><button type="button" className={selectedTags.includes(tag)?"selected":""} onClick={()=>toggleTag(tag)} key={tag}>#{tag}</button>)}</div></div>
    <div className="followup-grid"><label><span>保存后客户状态</span><select value={client.stage === "已成交" ? "已成交" : "待推荐"} disabled><option>{client.stage === "已成交" ? "已成交" : "待推荐"}</option></select></label><label><span>下次跟进时间 <i>*</i></span><input type="datetime-local" value={next} onChange={event=>setNext(event.target.value)} required /></label></div>
    <div className="quick-followup-time"><span>快速设置：</span>{[5,10,15,30].map(days=><button type="button" key={days} onClick={()=>setQuickNext(days)}>+{days}天</button>)}</div>
    <footer><button type="button" onClick={onClose}>取消</button><button className="primary" type="submit">{result.includes("待预约") || result.includes("安排带看") ? "保存并填写预约" : "保存跟进并转为待推荐"}</button></footer>
  </form></div>;
}
...
function ReminderSettingsModal({rules,onChange,onClose}:{rules:ReminderRules;onChange:(rules:ReminderRules)=>void;onClose:()=>void}) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="reminder-modal" role="dialog" aria-modal="true" aria-label="待办提醒规则" onMouseDown={event=>event.stopPropagation()}><header><div><h2>待办提醒规则</h2><p>启用或停用后，工作台风险数与超时队列会立即同步。</p></div><button className="close" aria-label="关闭弹窗" onClick={onClose}>×</button></header><div className="reminder-rule-list">{options.map(([key,title,description])=><label key={key}><div><b>{title}</b><span>{description}</span></div><input type="checkbox" checked={rules[key]} onChange={event=>onChange({...rules,[key]:event.target.checked})}/><i>{rules[key] ? "已启用" : "已停用"}</i></label>)}</div><footer><button onClick={onClose}>取消</button><button className="primary" onClick={onClose}>保存规则</button></footer></section></div>;
}
